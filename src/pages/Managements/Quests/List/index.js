...
const List = () => {
  ...
  const handleDelete = async (type, el) => {
    if (type === 1) {
      ...
      await request(REQUEST_TYPE.POST, 'task/delete', formData)
  ...
  const handleChange = async (el) => {
    ...
    await request(REQUEST_TYPE.POST, 'task/edit', formData)
    handleSubmit(null, data?.pagination?.page)
  }
  ...
  const handleSubmit = useCallback(async (e, page = 0, nextFilter = filter) => {
    ...
    setData(await request(REQUEST_TYPE.POST, 'quests/', formData))
  }, [filter, quantity])
    ...
    <>
      ...
      >
        ...
        <form onSubmit={(e) => handleSubmit(e, 0)}>
          ...
          <div className={style.actions}>
            <Reference
              to={`${NAVIGATION.managements.quests.link}/add`}
              classes={['primary']}
              placeholder={t('add')}
            />
          </div>
      ...
      <Paper>
        ...
        <div className={style.table}>
          <div className={style.row}>
            ...
            <div className={style.cell}>{t('bonus')}</div>
            <div className={style.cell}>{t('date_created')}</div>
            <div className={style.cell}>{t('expired_date')}</div>
              ...
              :
                data?.data?.map((el, idx) =>
                  ...
                  >
                    ...
                    <div className={style.cell}>
                      <Reference
                        to={`${NAVIGATION.managements.bonuses.link}/${el.bonus?.id}`}
                        classes={['outline', style.link]}
                        placeholder={el.bonus?.title}
                      />
                    </div>
