...
const List = () => {
  ...
  const handleDelete = async (type, el) => {
    if (type === 1) {
      ...
      await request(REQUEST_TYPE.POST, 'page/delete', formData)
  ...
  const handleChange = async (el) => {
    ...
    await request(REQUEST_TYPE.POST, 'page/edit', formData)
    handleSubmit(null, data?.pagination?.page)
  }
  ...
  const handleSubmit = useCallback(async (e, page = 0, nextFilter = filter) => {
    ...
    setData(await request(REQUEST_TYPE.POST, 'pages/', formData))
  }, [filter, quantity])
  ...
  return (
    <>
      <Breadcrumbs
        ...
        current={{text: NAVIGATION.managements.pages.text}}
      />
      <Paper
        headline={t(NAVIGATION.managements.pages.text)}
        classes={['sm']}
        quantity={quantity}
      ...
      >
        ...
        <form onSubmit={(e) => handleSubmit(e, 0)}>
          ...
          <div className={style.actions}>
            <Reference
              to={`${NAVIGATION.managements.pages.link}/add`}
              classes={['primary']}
              placeholder={t('add')}
            />
          </div>
      ...
      <Paper>
        ...
        <div className={style.table}>
          <div className={style.row}>
            ...
            <div className={style.cell}>{t('link')}</div>
              ...
              :
                data?.data?.map((el, idx) =>
                  ...
                  >
                    ...
                    <div className={style.cell}>{el.url}</div>
                    <div className={style.cell}>
                      <Icon
                        icon="fa-pencil"
                        alt="edit"
                        action={() => navigate(`${NAVIGATION.managements.pages.link}/${el.id}`)}
                      />
                      <Icon
                        classes={['warning']}
