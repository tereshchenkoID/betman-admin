...
const List = () => {
  ...
  const handleDelete = async (type, el) => {
    if (type === 1) {
      ...
      await request(REQUEST_TYPE.POST, 'challenge/delete', formData)
  ...
  const handleChange = async (el) => {
    ...
    await request(REQUEST_TYPE.POST, 'challenge/edit', formData)
    handleSubmit(null, data?.pagination?.page)
  }
  ...
  const handleSubmit = useCallback(async (e, page = 0, nextFilter = filter) => {
    ...
    setData(await request(REQUEST_TYPE.POST, 'challenges/', formData))
  }, [filter, quantity])
  ...
  return (
    <>
      <Breadcrumbs
        ...
        current={{text: NAVIGATION.managements.challenges.text}}
      />
      <Paper
        headline={t(NAVIGATION.managements.challenges.text)}
        classes={['sm']}
        quantity={quantity}
      ...
      >
        ...
        <form onSubmit={(e) => handleSubmit(e, 0)}>
          ...
          <div className={style.actions}>
            <Reference
              to={`${NAVIGATION.managements.challenges.link}/add`}
              classes={['primary']}
              placeholder={t('add')}
            />
          </div>
                  ...
                  >
                    ...
                    <div className={style.cell}>
                      <Icon
                        icon="fa-pencil"
                        alt="edit"
                        action={() => navigate(`${NAVIGATION.managements.challenges.link}/${el.id}`)}
                      />
                      <Icon
                        classes={['warning']}
