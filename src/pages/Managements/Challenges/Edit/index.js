...
const Edit = ({ id }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data, error } = await request(REQUEST_TYPE.POST, `challenge/${isAdd ? 'add' : 'edit'}`, formData)

    if (!error) {
      setFilter(data)

      if (isAdd) {
        navigate(NAVIGATION.managements.challenges.link)
      }
    }
  }

  const handleLoad = async () => {
    const { data } = await request(REQUEST_TYPE.GET, `challenge/${id}`)
    setFilter(data)
  }
            ...
            <div className={style.banner}>
              <div className={style.wrapper}>
                <div className={style.content}>
                  ...
                  <h2 className={style.subtitle}>{currentTranslation?.subtitle}</h2>
