...
const List = () => {
  ...
  const handleDelete = async (type, el) => {
    if (type === 1) {
      ...
      await request(REQUEST_TYPE.POST, 'seo/delete', formData)
  ...
  const handleChange = async (el) => {
    ...
    await request(REQUEST_TYPE.POST, 'seo/edit', formData)
    handleSubmit(null, data?.pagination?.page)
  }
  ...
  const handleSubmit = useCallback(async (e, page = 0, nextFilter = filter) => {
    ...
    setData(await request(REQUEST_TYPE.POST, 'seo/', formData))
  }, [filter, quantity])
    ...
    <>
      ...
      >
        ...
        <form onSubmit={(e) => handleSubmit(e, 0)}>
          ...
          <div className={style.actions}>
            <Reference
              to={`${NAVIGATION.managements.seo.link}/add`}
              classes={['primary']}
              placeholder={t('add')}
            />
          </div>
