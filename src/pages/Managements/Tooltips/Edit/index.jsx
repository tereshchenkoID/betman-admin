...
const Edit = ({ id }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data, error } = await request(REQUEST_TYPE.POST, `tooltip/${isAdd ? 'add' : 'edit'}`, formData)

    if (!error) {
      setFilter(data)

      if (isAdd) {
        navigate(NAVIGATION.managements.tooltips.link)
      }
    }
  }

  const handleLoad = async () => {
    const { data } = await request(REQUEST_TYPE.GET, `tooltip/${id}`)
    setFilter(data)
  }
            ...
            <div className={style.grid}>
              <div>
                <Field
                  ...
                  isRequired={true}
                />
                <p className={style.label}>Example: <strong>verification/rejected</strong></p>
              </div>
            </div>
            <Tab
              data={active}
