...
const AgentEdit = ({ mock }) => {
  ...
  const handleLoad = async (key = TABS[active].key) => {
    const { data, error } = await request(REQUEST_TYPE.GET, `agent/edit/${key}/${mock?.id}`)
  ...
  const handleSubmit = async (e, key = TABS[active].key) => {
    ...
    const { data, error } = await request(REQUEST_TYPE.POST, `agent/edit/${key}/${mock?.id}`, formData)
